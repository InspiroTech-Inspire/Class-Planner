...
function renderWeek(cls, week) {
  ...
  week.entries.forEach((entry) => body.appendChild(renderEntry(cls, week, entry)));
  renderAddEntryControl(cls, week, body);
...
function renderMain() {
  ...
  header.innerHTML = `
    ...
    <div class="progress-ring-wrap">
      <div class="progress-ring-label">Overall progress<strong>${total ? `${pct}% · ${done}/${total}` : "—"}</strong></div>
    </div>
  `;
